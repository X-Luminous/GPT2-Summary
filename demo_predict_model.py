...
from demo_util import format_data
...
def set_interact_args():
    ...
    parser.add_argument('--dialogue_model_path', default='summary_model/GPT2_NLPCC_Summary/', type=str, required=False, help='对话模型路径')
...
def predict_text(tokenizer,model,text,title_key,args,device):
...
def main(file_content_dict,output_file,pool_num = 5,encoding_str="utf-8"):
    pool = Pool(pool_num)
    ...
    p_list = []
    for title_key in tqdm(file_content_dict):
        #ind += 1
        #if ind >=6:
        #    continue
        content_list = file_content_dict.get(title_key,[])
        text = "".join(content_list)
        rslt = pool.apply_async(predict_text,(tokenizer, model, text,title_key, args, device))
        p_list.append(rslt)
    pool.close()
    pool.join()

    with io.open(output_file, "w", encoding=encoding_str) as f:
        for p_res in p_list:
            result_dict = {}
            try:
                title_key,content_list_new = p_res.get()
            except:
                print("error pool get:"+title_key)
                continue
            result_dict[title_key] = content_list_new
...
if __name__ == '__main__':
    ...
    mode_str = "news"
    encoding_str = "utf-8"
    base = "D:\\Projects\\PythonProject\\MindSharer_lm\\data_tzb_2023\\%s" % (mode_str)
    output_file = os.getcwd() + "/data/tzb2023/%s_summary.json" % (mode_str)
    file_content_dict = format_data(base)
    main(file_content_dict,output_file,pool_num )
